...
in
{
  ...
  networking.hostName = "ghuntley-dev";
  ...
  networking.firewall.interfaces."ens18".allowedTCPPorts = [ 22 ];
  ...
  systemd.services.backup = {
    ...
    script = ''
      ${pkgs.rsync}/bin/rsync --archive --verbose --human-readable --delete-after --stats --compress  -e "ssh -i ${config.age.secrets.rsync-net-backups-ssh-key.path}"  /depot zh2297@zh2297.rsync.net:machines/ghuntley.dev/
      ${pkgs.rsync}/bin/rsync --archive --verbose --human-readable --delete-after --stats --compress -e "ssh -i ${config.age.secrets.rsync-net-backups-ssh-key.path}" /srv zh2297@zh2297.rsync.net:machines/ghuntley.dev/
      ${pkgs.rsync}/bin/rsync --archive --verbose --human-readable --delete-after --stats --compress -e "ssh -i ${config.age.secrets.rsync-net-backups-ssh-key.path}" /etc zh2297@zh2297.rsync.net:machines/ghuntley.dev/
    '';
  };
