...
{ pkgs, ... }:
{
  services.uptermd = {
    ...
    extraFlags = [
      "--hostname"
      "upterm.ponderoos.com"
    ];
  };

  services.nginx.virtualHosts."upterm.ponderoos.com" =
    let
      css = pkgs.fetchurl {
    ...
    in
    {
      ...
      locations."/" = {
        root = pkgs.runCommand "webroot" { nativeBuildInputs = [ pkgs.pandoc ]; } ''
          ...
          ```
          upterm host --github-user 'githubusername' --server ssh://upterm.ponderoos.com:2323 --force-command 'tmux attach -t pair-programming' -- tmux new -t pair-programming
          ```
          ...
          Host:                   ssh://upterm.ponderoos.com:2323
          SSH Session:            ssh yXbrhy1bXaRIwTsgxJ8l:Wzo6XToyMjIy@upterm.ponderoos.com -p 2323
